/*
 * Created on Nov 18, 2004 by peterg CubicLine.java in edu.wpi.mqp.napkin.geometry
 * for MQP
 */
...
public class CubicLine extends Double implements UtilityShape {
	 ...
	 */
	public CubicLine(double x1, double y1, double ctlx1, double ctly1, double ctlx2,
	...
	/**
	 * This returns an approximation of the length of the cubic curve. The length
	 * is approximated as follows: A path consisting of joined line segments is
	 * formed such that the first segment connects point 1 with control point 1,
	 * the second segment connects control point 1 with control point 2, and the
	 * third segment connects control point 2 with point 2. The length of this
	 * path is then divided by (1 + log(flatness(this curve) + 1)).
	 * 
	...
	public UtilityShape magnify(double scaleFactor) {
		return new XMLCubicLine(this.x1 * scaleFactor, this.y1 * scaleFactor,
				this.ctrlx1 * scaleFactor, this.ctrly1 * scaleFactor, this.ctrlx2
	...
	public QuadLine[] transformToQuad() {
		...
		Point intersection = new StraightLine(this.getP1(), this.getP2())
				.intersects(new StraightLine(this.getCtrlP1(), this.getCtrlP2()));
		if (intersection == null) {
			ret = new QuadLine[1];
			ret[0] = new QuadLine(this.getP1(), Point.midpoint(this.getCtrlP1(), this
